...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        if sr_ratio > 1:
            self.act = nn.GELU()
            if sr_ratio == 8:
                self.sr1 = nn.Conv2d(dim, dim, kernel_size=8, stride=8,groups=dim)
                self.norm1 = nn.LayerNorm(dim)
                self.sr2 = nn.Conv2d(dim, dim, kernel_size=4, stride=4,groups=dim)
                self.norm2 = nn.LayerNorm(dim)
            ...
            if sr_ratio == 4:
                self.sr1 = nn.Conv2d(dim, dim, kernel_size=4, stride=4, groups=dim)
                self.norm1 = nn.LayerNorm(dim)
                self.sr2 = nn.Conv2d(dim, dim, kernel_size=2, stride=2, groups=dim)
                self.norm2 = nn.LayerNorm(dim)
                self.sr3 = nn.Conv2d(dim, dim, kernel_size=1, stride=1, groups=dim)
                self.norm3 = nn.LayerNorm(dim)
                self.sr4 = nn.Conv2d(dim, dim // 4 , kernel_size=3, stride=1, padding=1)
                self.norm4 = nn.LayerNorm(dim // 4)
                ...
                # self.kv4 = nn.Linear(dim, dim // 2, bias=qkv_bias)
            if sr_ratio == 2:
                self.sr1 = nn.Conv2d(dim, dim, kernel_size=2, stride=2,groups=dim)
                self.norm1 = nn.LayerNorm(dim)
                self.sr2 = nn.Conv2d(dim, dim, kernel_size=1, stride=1,groups=dim)
                self.norm2 = nn.LayerNorm(dim)
                self.sr3 = nn.Conv2d(dim, dim // 4, kernel_size=3, stride=1,padding=1)
                self.norm3 = nn.LayerNorm(dim // 4)
                self.sr4 = nn.Conv2d(dim, dim // 4, kernel_size=3, stride=1, padding=1,groups=dim//4)
                self.norm4 = nn.LayerNorm(dim // 4)
...
@register_model
def v2_shunted(pretrained=False, **kwargs):
