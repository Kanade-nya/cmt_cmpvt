...
----- update -----
-> v5_shunted_retina_1x
1. sr = 2, 第四个dw conv 改为 conv
2. 卷积部分 卷积核大小由 [5,5,3,3] 改为[3,3,5,5]
3. 增加一个 LPU()
4. Overloap Patch Embeding 改为普通的pE
5. 更换MLP
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        if sr_ratio > 1:
            self.act = nn.GELU()
            if sr_ratio == 8:
                self.sr1 = nn.Conv2d(self.dim, self.dim, kernel_size=8, stride=8)
                self.norm1 = nn.LayerNorm(self.dim)
                self.sr2 = nn.Conv2d(self.dim, self.dim, kernel_size=4, stride=4)
            ...
            if sr_ratio == 4:
                self.sr1 = nn.Conv2d(self.dim, self.dim, kernel_size=4, stride=4)
                self.norm1 = nn.LayerNorm(self.dim)
                self.sr2 = nn.Conv2d(self.dim, self.dim, kernel_size=2, stride=2)
                self.norm2 = nn.LayerNorm(self.dim)
                self.sr3 = nn.Conv2d(self.dim, self.dim, kernel_size=1, stride=1)
                ...
                # self.kv4 = nn.Linear(dim, dim // 2, bias=qkv_bias)
            if sr_ratio == 2:
                self.sr1 = nn.Conv2d(self.dim, self.dim, kernel_size=2, stride=2)
                self.norm1 = nn.LayerNorm(self.dim)
                self.sr2 = nn.Conv2d(self.dim, self.dim, kernel_size=1, stride=1)
                self.norm2 = nn.LayerNorm(self.dim)
...
@register_model
def v5_shunted(pretrained=False, **kwargs):
...
@register_model
def v5_shunted_light(pretrained=False, **kwargs):
    # sr_ratios = 【8，4，2，1】 num_heads = 【1，2，5，8】
    model = REPVTModel(
        patch_size=4,
...
@register_model
def v5_shunted_middle(pretrained=False, **kwargs):
    # sr_ratios = 【8，4，2，1】 num_heads = 【1，2，5，8】
    model = REPVTModel(
        patch_size=4,
        embed_dims=[64,128,256,512],
...
@register_model
def v5_max(pretrained=False, **kwargs):
    # sr_ratios = 【8，4，2，1】 num_heads = 【1，2，5，8】
    model = REPVTModel(
        patch_size=4,
        embed_dims=[64, 128, 256, 512],
