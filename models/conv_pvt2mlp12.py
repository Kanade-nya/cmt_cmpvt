...
__all__ = [
    'conv_pvt2mlp_tiny12', 'conv_pvt2mlp_tiny_chaoyang', 'conv_pvt2mlp_tiny_flowers', 'conv_pvt2mlp_tiny_imagenet'
]
...
class PvtConv(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chanels=3, dim=128, num_classes=1000, embed_dims=[128, 256, 512],
        ...
        self.block1 = nn.Sequential(
            nn.Conv2d(in_channels=n_fliter_list[0], out_channels=64, kernel_size=7, stride=2, padding=3),
            # 112*112 变成1/2
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            ...
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels=128, out_channels=embed_dims[0], kernel_size=3, stride=2, padding=1),  # 28
            nn.BatchNorm2d(embed_dims[0]),
            nn.ReLU(inplace=True),
            torch.nn.Conv2d(in_channels=embed_dims[0], out_channels=embed_dims[0], stride=1, kernel_size=3, padding=1),  # 28
            nn.BatchNorm2d(embed_dims[0]),
            nn.ReLU(inplace=True),
        )  # endregion
...
@register_model
def conv_pvt2mlp_tiny12(pretrained=False, **kwargs):
    model = PvtConv(embed_dims=[128, 320, 512],
                    num_classes=100,
                    num_heads=[2, 4, 8],
                    mlp_ratios=[8, 4, 4],
                    qkv_bias=True,
...
@register_model
def conv_pvt2mlp_light(pretrained=False, **kwargs):
    model = PvtConv(embed_dims=[96,240,384],
                    num_classes=100,
                    num_heads=[4, 8, 16],
                    mlp_ratios=[8, 4, 4],
                    qkv_bias=True,
